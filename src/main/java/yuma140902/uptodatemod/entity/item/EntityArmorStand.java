...
public class EntityArmorStand extends EntityLiving {
	...
	public ItemStack getPickedResult(MovingObjectPosition target) {
		return new ItemStack(MyItems.armorStand);
	}
	...
	public boolean interact(EntityPlayer player) {
		if (worldObj.isRemote) {
			ModUpToDateMod.networkWrapper.sendToServer(new ArmorStandInteractMessage(worldObj.provider.dimensionId, this, player));
			return true;
		}
		return false;
	}
	...
	public boolean interact(EntityPlayer player, Vec3 hitPos) {
		if (!worldObj.isRemote) {
			...
			boolean notNullFlag = itemstack != null;

			if (notNullFlag && itemstack.getItem() instanceof ItemArmor) {
			...
			if (notNullFlag && (itemstack.getItem() == Items.skull || itemstack.getItem() == Item.getItemFromBlock(Blocks.pumpkin)))
				b0 = 4;
			...
			if (notNullFlag && b0 == 0 && !getShowArms())
				return true;
			else {
				if (notNullFlag)
	...
	private void dropBlock() {
		entityDropItem(new ItemStack(MyItems.armorStand), 0.0F);
		dropequipment();
	}
