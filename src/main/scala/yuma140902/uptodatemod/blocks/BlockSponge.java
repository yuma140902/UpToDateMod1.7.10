...
public class BlockSponge extends Block implements IRegisterable, IHasRecipes {
    ...
    private static boolean absorb(World world, BlockPos pos) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (final ForgeDirection direction : ForgeDirection.VALID_DIRECTIONS) {
                BlockPos blockpos = blockPosOrig.offset(direction, 1);
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random rand) {
        if (world.getBlockMetadata(x, y, z) == META_WET) {
            ...
            world.spawnParticle(McConst.Particle$.MODULE$.DRIP_WATER(), posX, posY, posZ, velX, velY, velZ);
        }
    }
    ...
    public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase entity, ItemStack stack) {
        ...
        if (meta == META_WET && world.provider.dimensionId == -1) {
            ...
            for (int i = 0; i < rand.nextInt(10) + 5; ++i) {
                ...
                double velY = rand.nextDouble() * 0.05D;
                world.spawnParticle(McConst.Particle$.MODULE$.CLOUD(), posX, posY, posZ, 0, velY, 0);
            }
        } else {
