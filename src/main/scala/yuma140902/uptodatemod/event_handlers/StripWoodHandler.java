...
public class StripWoodHandler {
    ...
    @Nonnull
    private static final List<IWoodStrippingInfo> registry = new ArrayList<>();
    ...
    public static void handle(PlayerInteractEvent event) {
        ...
        BlockWithMetadata state = ScalaCompat.optionOrNull(BlockWithMetadata.fromCoordinate(world, new BlockPos(x, y, z)));
        for (final IWoodStrippingInfo info : registry) {
            ...
            if (soundName != null)
                world.playSoundEffect(x + 0.5D, y + 0.5D, z + 0.5D, soundName, 1.0F, world.rand.nextFloat() * 0.1F + 0.9F);
        }
